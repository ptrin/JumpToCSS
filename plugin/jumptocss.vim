...
# regexen used to get the parts of the start tag
regexen = {
    'tag': {
        'regex':'<([a-zA-Z0-9-_]+)(?:[^>])',
        'compiled_re':None,
...
def get_start_tag():
    try:
        ...
        if z_reg is not None:
            vim.command('let @z="'+escape(z_reg)+'"')
...
while True:
    ...
    if start_tag is None:
        vim.command('echo "JumpToCSS: error"')
        break
    ...
    # based on the "parts" available in the current line of the HTML buffer, 
    # create the regex to use to search CSS buffers
